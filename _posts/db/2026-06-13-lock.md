...
## <span style="color:#802548">_page vs row vs table lock_</span>
- Table-Level Lock: Locks the entire table structure and all its data.
    - The database places a single lock entry in its central memory lock manager
    - this is logical lock, it doesnt mean all row lock
- Row-Level Lock: Locks one specific record, maximizing multi-user concurrency.
    - oracle: ITL slot header in page data area 
    - mysql: data page's transaction ID
    - SQL server: The Centralized Lock Manager
- page/ Level Lock: Locks a physical chunk of memory or disk storage containing multiple records.
    - when DML being executed, Physical Latch is triggered
        - The DB acquires a Logical Row Lock on Row 1 (lasts until you type COMMIT).
        - The DB triggers a Physical Latch (Page Lock) on the 8 KB memory page containing the row 
        - The CPU changes the binary bits of Row 1 inside the RAM
        - The DB immediately releases the Physical Latch on the page 
    - it only lasts for microseconds


- One TABLE consists of Many PAGES
- One PAGE contains Many ROWS

```text
┌────────────────────────────────────────────────────────┐
│                      TABLE                             │ 
│  (Logical concept: The definition of your data)        │
└───────────────────────────┬────────────────────────────┘
                            │ (is physically stored in...)
                            ▼
┌────────────────────────────────────────────────────────┐
│                DATABASES / TABLESPACES                 │
│  (The physical storage space on your disk/SSD)         │
└───────┬───────────────────┬────────────────────┬───────┘
        │                   │                    │
        ▼                   ▼                    ▼
 ┌──────────────┐    ┌──────────────┐     ┌──────────────┐
 │    PAGE 1    │    │    PAGE 2    │     │    PAGE 3    │  <-- PHYSICAL BOXES
 │ (Block 8KB)  │    │ (Block 8KB)  │     │ (Block 8KB)  │      (Fixed Size)
 └──────┬───────┘    └──────┬───────┘     └──────┬───────┘
        │                   │                    │
  ┌─────┴─────┐             │              ┌─────┴─────┐
  ▼           ▼             ▼              ▼           ▼
┌───────┐   ┌───────┐   ┌───────┐        ┌───────┐   ┌───────┐
│ Row 1 │   │ Row 2 │   │ Row 3 │        │ Row 4 │   │ Row 5 │  <-- LOGICAL ROWS
└───────┘   └───────┘   └───────┘        └───────┘   └───────┘   (Actual Data)
```
...
## <span style="color:#802548">_how to see locked row and table_</span>